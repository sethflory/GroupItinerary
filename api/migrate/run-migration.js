#!/usr/bin/env node
/**
 * Standalone migration script - run with Node.js
 *
 * Usage:
 *   cd api
 *   node migrate/run-migration.js [options]
 *
 * Options:
 *   --dry-run     Preview what will be created without writing
 *   --clean       Delete all existing data before migrating
 *   --trip=ID     Only migrate a specific trip (athens-bangalore-2026 or test-trip)
 *
...
const TRIP_FILTER = args.find(a => a.startsWith('--trip='))?.split('=')[1];
...
if (!connectionString) {
  ...
  console.error("Set it via environment variable or create api/.env file:");
...
// Stats tracking
const stats = {
  tables: { created: 0, existed: 0, failed: 0 },
  entities: { created: 0, updated: 0, failed: 0, deleted: 0 },
  errors: []
};

// =============================================
// FULL HARDCODED DATA FROM index.html
// =============================================
...
const PRODUCTION_PHASES = {
  together: { name: 'Together', days: [0, 1, 2, 3, 4, 5], travelers: ['all', 'seth', 'katrina', 'paul', 'sheri'] },
  india: { name: 'S&P India', days: [5, 6, 7, 8, 9, 10, 11], travelers: ['all', 'seth', 'paul'] }
};

const TEST_PHASES = {
  together: { name: 'Test Phase', days: [0, 1, 2], travelers: ['all', 'tester1', 'tester2'] }
};
...
// FULL 12-day production itinerary
const PRODUCTION_DAYS = [
  ...
  },
  {
    ...
    events: [
      ...
      { id: "dinner-day5", time: "20:00", type: "meal", title: "Group Dinner", subtitle: "Last night together", travelers: ["all"], status: "pending", where: "TBD", details: "Last dinner with everyone before K&S head home and S&P continue to India." }
    ]
  },
  {
...
async function deleteTableContents(tableName, filter) {
  if (DRY_RUN) {
    console.log(`  [dry-run] Would delete from: ${tableName}`);
    return;
  }
  ...
  try {
    ...
    for await (const entity of client.listEntities(filter ? { filter } : {})) {
    ...
    if (entities.length > 0) {
      console.log(`  ✓ Deleted ${entities.length} entities from ${tableName}`);
    }
  } catch (err) {
    if (err.statusCode !== 404) {
      console.log(`  ⚠ Could not clean ${tableName}: ${err.message}`);
    }
  }
}
...
async function upsertEntity(tableName, entity) {
  ...
  try {
    ...
    console.log(`  ✗ Failed to upsert ${entity.rowKey}: ${err.message}`);
    stats.entities.failed++;
    stats.errors.push({ table: tableName, entity: entity.rowKey, error: err.message });
  }
}

async function migrate() {
  console.log("\n" + "=".repeat(50));
  console.log("  GROUP ITINERARY - DATABASE MIGRATION");
  ...
  if (TRIP_FILTER) {
    console.log(`📋 Migrating only: ${TRIP_FILTER}\n`);
  }
  ...
  if (CLEAN) {
    ...
    await deleteTableContents(TABLES.DESTINATIONS);
    for (const tripId of Object.keys(TRIPS)) {
      if (TRIP_FILTER && tripId !== TRIP_FILTER) continue;
      await deleteTableContents(TABLES.TRIPS, `RowKey eq '${tripId}'`);
      await deleteTableContents(TABLES.TRAVELERS, `PartitionKey eq '${tripId}'`);
      await deleteTableContents(TABLES.DAYS, `PartitionKey eq '${tripId}'`);
      await deleteTableContents(TABLES.EVENTS, `PartitionKey ge '${tripId}_' and PartitionKey lt '${tripId}~'`);
      await deleteTableContents(TABLES.HOTELS, `PartitionKey eq '${tripId}'`);
    }
  }

  // Migrate destinations (global)
  if (!TRIP_FILTER) {
    console.log("\n🌍 Migrating destinations...");
    for (const [code, dest] of Object.entries(DESTINATIONS)) {
      await upsertEntity(TABLES.DESTINATIONS, {
        partitionKey: "destinations",
        rowKey: code,
        ...dest
      });
      if (!DRY_RUN) console.log(`  ✓ ${code}: ${dest.city}`);
    }
  }
  ...
  for (const [tripId, trip] of Object.entries(TRIPS)) {
    ...
    if (!DRY_RUN) console.log(`  ✓ Trip config`);

    // Travelers
    for (const traveler of travelers) {
      await upsertEntity(TABLES.TRAVELERS, {
        partitionKey: tripId,
        rowKey: traveler.id,
        ...traveler,
        triviaScore: 0
    ...
    if (!DRY_RUN) console.log(`  ✓ ${travelers.length} travelers`);

    // Hotel
    await upsertEntity(TABLES.HOTELS, {
      partitionKey: tripId,
      rowKey: hotel.id,
      ...hotel
    });
    if (!DRY_RUN) console.log(`  ✓ Hotel: ${hotel.name}`);

    // Days and Events
    let eventCount = 0;
    for (const day of days) {
      await upsertEntity(TABLES.DAYS, {
        partitionKey: tripId,
        rowKey: day.date,
        dayNum: day.dayNum,
        label: day.label,
      ...
      for (const event of day.events || []) {
        await upsertEntity(TABLES.EVENTS, {
    ...
    if (!DRY_RUN) console.log(`  ✓ ${days.length} days, ${eventCount} events`);
  }
  ...
  console.log("=".repeat(50));
  console.log(`\n  Tables:   ${stats.tables.created} created, ${stats.tables.existed} existed, ${stats.tables.failed} failed`);
  console.log(`  Entities: ${stats.entities.created} upserted, ${stats.entities.deleted} deleted, ${stats.entities.failed} failed`);

  if (stats.errors.length > 0) {
    console.log("\n⚠️  Errors:");
    stats.errors.forEach(e => console.log(`    - ${e.table}/${e.entity || ''}: ${e.error}`));
  }
